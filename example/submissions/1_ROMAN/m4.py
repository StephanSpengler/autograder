...
def age():
    ...
    l = len(ages)
    while l>1:
        answer = input(f"Is your age less than {ages[l//2]}?: ")
        if answer == "no":
            ages = ages[(l//2):]
        elif answer == "yes":
            ages = ages[:(l // 2)]
...
def sqrt(x):
    if x < 0:
        raise EvaluationError(f'Illegal argument to sqrt: {x}')
    return math.sqrt(x)

def fib(n, memory):
    if n<0 or n//1!=n:
        raise EvaluationError(f'Illegal argument to fib: {n}')
    if n not in memory:
        memory[n] = fib(n - 1, memory) + fib(n - 2, memory)
    return memory[n]

FUNCTIONS_0 = {  # Functions with no argument
    'age': age,
...
def main():
    ...
    variables = {"ans": 0.0}
    memory = {0: 0, 1 :1}
    FUNCTIONS_1['fib'] = lambda n: fib(n, memory)
